...
| `locate.ts` | `surface` → 원고 위치. 정확 일치 → 공백 접기 → 3단계(공백 전부 제거 + 곱은따옴표→곧은따옴표 + …→..., 위치 매핑 유지). 셋 다 정확 일치이고 유사 일치는 없다. 못 찾으면 폐기(`dropped`). 원칙 6의 문지기 | 없음 |
| `conflicts.ts` | 검사 ①(전이 없는 변화) + ①′(전이 뒤 옛 값 재등장). 위치는 (회차, 오프셋). 입력 순서 무관 | 없음 |
| `conflicts.test.ts` | 물기 시험 29건 — 골격 10 + 리뷰 탐침 P1~P7 + 정규화·시간 + locate 3단계 P8 | 없음 |
| `golden.ts` | 골든의 zod 계약(strict). 키 오타가 "조건 없음" 으로 조용히 통과하지 않게 로드 시 `safeParse` | 없음 |
...
아직 없는 것: 추출 호출 스크립트(`scripts/extract-setting.ts` — ai-probe 와 같은 하니스 경로. **이 픽스처에 대해서는 `{{branches}}` 로 작품 설정 `main / pre_regression` 을 넘긴다**, 모델이 갈래 id 를 짓지 않는다), lexicon 대조(같은 이름/별칭 → 같은 개체; 지금은 verify 안의 단순 매핑), 저장소 어댑터(관찰/정본 분리, 설계 §4-2 개정), 검사 ②③④, 작품별 속성 별칭 UI.
...
npx tsx lib/setting/conflicts.test.ts                       # 29 / 0
...
npx tsx lib/setting/verify.ts <ep1> <ep2> <ep1_run2> <ep2_run2>   # + 결정성
```
