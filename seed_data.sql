...
select
  ...
  '강태가 주먹을 휘둘렀다. 퍽! 주원이 막았다. 강태가 다시 휘둘렀다. 퍽! 퍽! 주원이 뒤로 물러났다. 강태의 주먹은 강력했다. 주원은 다시 앞으로 나갔다.', null, '{"maxChars":200,"minVerbs":4,"requireAll":["주원","강태"],"forbidLabel":"느낌을 말로 대신하는 표현","forbidWords":["끔찍","무서웠","두려웠","압도적","굉장","엄청난","강력","그때였다","과연"],"forbidDisplay":["끔찍하다","무섭다","두렵다","압도적","굉장하다","엄청나다","강력하다","그때였다","과연"],"forbidPassageCopy":true,"ai_shadow":"support"}'::jsonb,
...
select
  ...
  '정후는 검을 낮게 늘어뜨리고 걸어 들어왔다. 상단도 중단도 아닌 하단이었다. 노고수 백리진은 그 자세를 서른 해 전에 본 적이 있었다.', null, '{"maxChars":200,"minVerbs":4,"requireAll":["정후","백리진"],"forbidLabel":"느낌을 말로 대신하는 표현","forbidWords":["끔찍","무서웠","두려웠","압도적","굉장","엄청난","강력","그때였다","과연"],"forbidDisplay":["끔찍하다","무섭다","두렵다","압도적","굉장하다","엄청나다","강력하다","그때였다","과연"],"forbidPassageCopy":true,"ai_shadow":"support"}'::jsonb,
  'original', 'martial', 'planned',
  2, 'bt-low-guard'
...
-- ── 비활성 ─────────────────────────────────────────────────────────
-- 옛 action_turn convert 8건(재설계안 11-4·세션 18. 재료·픽스처는 11-5 픽스처 갈아엎기 때 삭제) + 구성 12 재설계로 밀려난 대비형 cc- 4건(세션 32 후기 — '입체 캐릭터'로 교체. cc-first-pay 는 활성 유지) + ig-ball-envelope(세션 34 정정 v2 — 손등 키스 예법 오류·억지 구도로 반려, ig-friend-text 로 교체). 제출 이력 보존, 행 삭제 안 함.

update problems set is_active = false
 where source_key in ('at-left-feint', 'at-left-draw', 'at-left-feeler', 'at-broken-gate', 'at-cracked-ice', 'at-bell-rope', 'at-look-back', 'at-edit-log', 'cc-report-credit', 'cc-street-night', 'cc-raid-reward', 'cc-relic-box', 'ig-ball-envelope');
