...
select
  (select id from stages where skill_key = 'contrast_char'),
  'convert', 'auto', '조평과 유겸을 대비시키시오. 조평은 배곯던 시절이 몸에 남아 제 것엔 인색해도 남의 끼니엔 아깝지 않은 호위다. 유겸은 신세를 지면 도련님 취급이 진짜가 될까 밥값부터 제가 내는 신참이다. 아래 장면을 다시 써서, 같은 삯 앞에서 두 사람이 서로 다르게 움직이게 하시오.',
...
select
  ...
  '월례 회의에서 이달의 우수 사원이 발표되었다. 호명된 대리는 앞으로 나가 상을 받았다.', null, '{"maxChars":100,"minVerbs":3,"forbidLabel":"속마음을 직접 말하는 표현","forbidWords":["좋아","기쁘","기뻤","뿌듯"],"forbidDisplay":["좋아하다","기쁘다","뿌듯하다"],"requireAny":["서담"],"forbidPassageCopy":true}'::jsonb,
  'original', 'modern', 'planned',
  1, 'cc-praise-callout'
...
select
  (select id from stages where skill_key = 'contrast_char'),
  'convert', 'auto', '도현의 겉과 속을 한 장면에 담으시오. 도현은 누구에게나 같은 온도로 친절한 S급 헌터다. 무전 속 부상자는 그가 마음의 울타리 안에 둔 몇 안 되는 사람이다. 아래 장면을 다시 써서, 흐트러지지 않는 겉과 일정을 처음 미루는 속이 둘 다 행동으로 보이게 하시오.',
  '레이드 중에 부상자가 나왔다는 무전이 들어왔다. 에이스는 예정된 인터뷰 장소로 이동하는 중이었다.', null, '{"maxChars":100,"minVerbs":3,"forbidLabel":"속마음을 직접 말하는 표현","forbidWords":["걱정","불안","초조"],"forbidDisplay":["걱정","불안","초조"],"requireAny":["도현"],"forbidPassageCopy":true}'::jsonb,
  'original', 'modern', 'planned',
  1, 'cc-ace-siren'
...
select
  ...
  '밤길 호위 순번을 정하는 자리였다. 가장 험한 새벽 구간은 아무도 맡으려 하지 않았다.', null, '{"maxChars":100,"minVerbs":3,"forbidLabel":"속마음을 직접 말하는 표현","forbidWords":["불안","자존심","증명"],"forbidDisplay":["불안","자존심","증명하다"],"requireAny":["유겸"],"forbidPassageCopy":true}'::jsonb,
  'original', 'martial', 'planned',
  1, 'cc-night-shift'
...
select
  ...
  '고물상이 유품 값을 반으로 후려쳤다. 견습들은 물건을 다시 싸서 돌아갈 채비를 했다.', null, '{"maxChars":100,"minVerbs":3,"forbidLabel":"속마음을 직접 말하는 표현","forbidWords":["소중","다정"],"forbidDisplay":["소중하다","다정하다"],"requireAny":["셀라"],"forbidPassageCopy":true}'::jsonb,
  'original', 'fantasy', 'planned',
  1, 'cc-junk-dealer'
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 1, '', '호명된 서담은 손사래를 치며 반쯤 일어섰다. 팀장님이 다 하신 건데요, 하는 목소리가 끝까지 작았다. 그런데 자리로 돌아온 뒤에도 상장은 서류철에 못 들어갔다. 모니터 옆에 세워 둔 채, 서담은 오후 내내 그쪽으로 눈을 안 주는 척했다.'
from problems p
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 2, '', '서담은 상을 받는 내내 고개를 숙이고 있었다. 회식 자리에서 그 얘기가 나오자 화제부터 돌렸다. 그런데 그날 밤, 서담의 책상 서랍에는 상장이 반듯하게 눕혀졌다. 모서리가 구겨질까 봐 파일까지 끼운 채였다.'
from problems p
where p.source_key = 'cc-praise-callout'
on conflict (problem_id, ord, blank_key) do nothing;
...
-- cc-ace-siren ord 1 
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 1, '', '도현은 남은 질문 둘에 끝까지 답했다. 목소리는 평소의 온도 그대로였다. 인터뷰가 끝나자 그는 처음으로 다음 일정을 미뤘다. 기자에게 양해를 구하는 인사도, 병원으로 방향을 트는 걸음도 흐트러지지 않았다.'
from problems p
...
-- cc-ace-siren ord 2 
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 2, '', '무전이 끝나고도 도현의 말씨는 평소 그대로였다. 같은 온도의 경어로 인터뷰 연기를 부탁했다. 그러나 주차장까지 걸어 내려가는 법이 없던 사람이 그날은 엘리베이터를 기다리지 못했다. 계단을 두 칸씩 내려가는 발소리가 울렸다.'
from problems p
where p.source_key = 'cc-ace-siren'
on conflict (problem_id, ord, blank_key) do nothing;
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 1, '', '도련님이 새벽 경계도 서겠냐는 웃음 섞인 말이 돌았다. 유겸은 더 크게 웃으며 명부에 제 이름을 적었다. 그게 얼마나 한다고요, 하는 목소리도 가벼웠다. 그러나 돌아서서 붓을 내려놓는 손등에는 힘줄이 서 있었다.'
from problems p
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 2, '', '유겸은 웃는 낯으로 새벽 구간에 손을 들었다. 잠이 안 와서 그런다며 너스레까지 떨었다. 그런데 그날 밤 순찰로를 도는 걸음은 평소의 배로 촘촘했다. 담장 아래 어둠을 살피는 눈에는 웃음기가 없었다.'
from problems p
where p.source_key = 'cc-night-shift'
on conflict (problem_id, ord, blank_key) do nothing;
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 1, '', '셀라는 고물상의 값을 듣자마자 목록을 덮었다. 은화 열 닢 아래로는 안 팝니다, 하는 목소리에 흥정의 여지가 없었다. 돌아가는 길, 셀라는 수레 위 유품 보자기를 다시 여몄다. 매듭이 풀리지 않게 두 번을 더 조인 손이었다.'
from problems p
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 2, '', '셀라는 물건을 도로 싸며 값을 두 번 말하지 않았다. 장부에는 깨진 흥정의 값만 건조하게 적혔다. 다만 상자를 드는 리안의 손이 느려지자, 셀라는 제 몫의 짐을 먼저 지고 앞서 걸었다. 재촉하는 말 대신 걸음만 늦췄다.'
from problems p
where p.source_key = 'cc-junk-dealer'
on conflict (problem_id, ord, blank_key) do nothing;
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 1, '', '카메라 앞으로 헌터들이 줄을 이뤘다. 한시우는 그 줄의 끝을 지나쳐 장비부터 쌌다. 인터뷰 소리가 등 뒤에서 커졌다 작아졌다. 훈련장 예약까지 십 분, 한시우의 걸음은 게이트 반대쪽으로 이미 멀어지고 있었다.'
from problems p
...
insert into reference_answers (problem_id, ord, blank_key, content)
select p.id, 2, '', '플래시가 터질 때마다 사람들의 고개가 그쪽으로 쏠렸다. 한시우만 무너진 방벽 앞에 쪼그려 앉아 있었다. 제 검이 낸 흔적을 손끝으로 재고, 수첩에 각도를 옮겨 적었다. 등 뒤의 환호는 끝까지 한 번도 돌아보지 않았다.'
from problems p
where p.source_key = 'cc-flash-crowd'
on conflict (problem_id, ord, blank_key) do nothing;
