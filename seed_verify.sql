...
-- 알려준다.
--
-- 마지막 문장이 select 인 것은 의도다. Supabase 편집기가 NOTICE 를
-- 안 띄워서, raise notice 로 끝내면 "통과"와 "파일이 잘려 안 돌았다"가
-- 둘 다 'Success. No rows returned' 로 보인다. 행이 나오면 끝까지 돈 것이다.
-- 이 select 뒤에 다른 문장을 두지 마라 — 편집기는 마지막 결과만 보여준다.
-- 맨 아래 "확인 쿼리"는 전부 주석이라 실행되지 않는다.
-- =====================================================================
...
begin
  ...
  --       구성 15 info_gap 무난 원문 3건(세션 34 정정 v2 — ig-left-cup·
  --       ig-umbrella-walnut·ig-friend-text, 셋 다 type continue라 이미 안
  --       걸리지만 명시로도 뺀다) + ig-gate-wait 1건(type convert — 이건 진짜로
