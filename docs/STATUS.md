...
나  scoring_config 표시 전용 필드 둘 — forbidLabel(범주 한 줄) · forbidDisplay(기본형 배열).
    있으면 규칙 줄 = forbidLabel + [전체 보기]. 기본형 전체는 '전체 보기'를 앵커로 한
    absolute 팝오버(relative 부모 + absolute 카드 · 배경·테두리·그림자 · 오른쪽 정렬 · zIndex 30 ·
    바깥클릭·Esc·접기로 닫힘) — 문서 흐름 밖이라 펼쳐도 아래 행이 안 밀린다. '예:' 미리보기 없음.
    새 컴포넌트 RuleText 를 '무엇을 봅니다'와 CheckRow 가 공유(fail 때 걸린 단어 칩은 별도,
    안 건드림). local.ts forbidWords 검사 + index.ts mergeForbidChecks
