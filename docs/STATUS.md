...
구성 16(ca-)의 signal 정의(세션 48)  16 의 신호 = 독자에게 주어진 사실(감각·사물·타인 행동)이
                              ...
                              걸음 붙어 걸음).
짧은 결론형 재료는 복사되고 두 줄 재료는 소화된다(세션 49 실측)  재료 복사 경보(세션 49
                              4-A) 실측 — 짧은 결론형 재료(한 줄, "~은 ~다"류) 5건이 힌트
                              본문에 그대로 베껴 나왔다(orc-axe 3 · fireball 2). bt-spear-range
                              (박 님이 세션 49 에 확정한 두 줄 재료)는 0/10 — 재료를 그대로
                              베끼지 않고 소화해 썼다. bt- 나머지 넷(fireball·alley-hook·
                              orc-axe·low-guard)의 재료를 두 줄로 확장할 근거가 이걸로 섰다
                              (열린 관찰 참고 — 아직 착수 안 함).
```
...
1  (닫힘, 세션 48 후속) signal-v2 판정선 실측 통과 — 구성 16(ca-) signal 실사용 확정
                        ...
                        없음 확인. 같은 세션 --hint 재측정에서 50건 중 32건이 {"feedback": "…"}
                        꼴 JSON 껍데기로 왔는데 verifyHintV3 다섯 제약을 전부 통과해(통과율
                        100%가 껍데기를 안 센 수치였다) 세션 49 후속(껍데기 벗기기)의 계기가
                        됐다.
                        ★ 세션 49 --hint 결과(재측정, 껍데기 벗기기 적용 후): 통과율 100% ·
                        껍데기 43/50 · 재료 복사 5/50(orc-axe 3 · fireball 2) · spear-range
                        0/10(두 줄 재료는 그대로 안 베낀다 — 짧은 결론형 재료가 복사되고 두
                        줄 재료는 소화된다는 게 위 '정한 것'으로 확정). 이 실측이 세션 50
                        의 계기다 — 36번({"message":"…"}, 키가 feedback 이 아니라 벗기기를
                        피함)·44번("...2번 앞에..." 문장 번호 누출)이 통과율 100% 뒤에 숨어
                        있었다.
                        ★ 세션 50 이 둘 다 코드로 막았다 — ① 벗기기 규칙을 키 이름과 무관하게
                        (문자열 필드 정확히 하나면 벗긴다) 넓혔다 ② verifyHintV3 여섯째 제약
                        (문장 번호 노출)을 추가하고, computeHintV3 에 재시도 1회 + 실패 시
                        'discarded' 캐시를 새로 달았다(학습자 화면은 어느 쪽이든 힌트 없이
                        1층 문구만 — 진도 무영향). 판정선(박 님 문구): "재측정의 목적은 0 을
                        확인하는 것이 아니다 — 두 구멍은 코드가 막으므로 화면은 재측정과
                        무관하게 안전하다. 목적은 고친 뒤에도 본문 결이 그대로인지 한 번 더
                        읽는 것이다. 다음에 1건이 튀면 '코드가 잡았나'만 본다."
                        다음 순서(박 님 절차, 5-A): ① push → 채팅 리뷰 ② `npm run
                        ai:support-golden -- --hint`($0.18 안팎, DB 작업 없음) ③ 본문 50건을
                        읽는다 — 결이 그대로인지만 본다 ④ 결이 그대로면 채팅이 hint_visible=
                        true SQL 한 줄을 별도로 낸다(새 커밋 없음, seed/update-hint-visible.sql
                        값만 바꾸는 델타). 그 다음 ⑤ 재제출 비교 피드백(설계 미착수) → ⑥ 보스
                        (아래 4번).
...
재료가 읽기만 주고 대가·선택을 안 준다 — bt- 나머지 넷(fireball·alley-hook·orc-axe·
  low-guard)도 spear-range 처럼 두 줄로 갈지 미결(세션 49). 세션 49 재측정 근거가
  섰다(위 '정한 것' — 짧은 결론형 재료 5/50 복사 vs spear-range 두 줄 0/10) —
  나머지 넷을 두 줄로 늘릴지는 아직 착수 안 했다. 세션 50 은 이 재료들을 안
  건드렸다(DB 작업 없음).
...
hint_visible 은 재측정 뒤 결정(세션 50 이 더 가까워졌다) — 세션 50 이 코드로
  두 구멍(키 이름 무관 벗기기·문장 번호 재시도+discarded 캐시)을 막았다. 판정선
  (박 님 문구): "재측정의 목적은 0 을 확인하는 게 아니다 — 두 구멍은 코드가
  막으므로 화면은 재측정과 무관하게 안전하다. 목적은 고친 뒤에도 본문 결이
  그대로인지 한 번 더 읽는 것이다." 박 님 절차(5-A) — push → 채팅 리뷰 →
  `npm run ai:support-golden -- --hint`(DB 작업 없음, $0.18 안팎) → 본문 50건
  읽기 → 결이 그대로면 hint_visible=true SQL 한 줄(새 커밋 없음).
힌트 v3 프롬프트에 평문 강제(JSON 금지) 추가 — 세션 49·50 둘 다 일부러 안
  넣었다(재료·검증 변경과 같은 커밋에서 프롬프트를 바꾸면 재측정이 뭘 잰 건지
  흐려진다). 세션 50 실측으로 기준선이 잡혔다 — 통과율 100% · 껍데기 43/50 ·
  재료 복사 5/50 · spear-range 0/10. 벗기기·문장 번호 차단을 코드로 넓힌
  뒤에도 결이 그대로인지 다음 재측정(위 hint_visible 절차)으로 확인하고, 그
  다음 세션에서 프롬프트에 평문 강제를 넣는다.
