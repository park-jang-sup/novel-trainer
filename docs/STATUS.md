...
짧은 결론형 재료는 복사되고 두 줄 재료는 소화된다(세션 49·50 실측, 두 번 재현)  재료 복사
                              ...
                              베끼지 않고 소화해 썼다. bt- 나머지 넷(fireball·alley-hook·
                              orc-axe·low-guard)의 재료를 두 줄로 확장할 근거는 이걸로 섰다.
...
재료가 읽기만 주고 대가·선택을 안 준다 — bt- 나머지 넷(fireball·alley-hook·orc-axe·
  low-guard)도 spear-range 처럼 두 줄로 갈지 미결(세션 49). 근거가 두 세션으로
  섰다(위 '정한 것' — 세션 49 5/50·세션 50 11/50 복사 vs spear-range 두 줄은
  두 세션 다 0/10) — 나머지 넷을 두 줄로 늘릴지는 아직 착수 안 했다. **확장은
  별도 세션에서 --hint 재측정과 묶어서 한다**(두 줄 재료가 #30형 위험을 재료
  쪽에서 부를 수 있어 이번 커밋들엔 안 섞었다, 박 님). 세션 51 도 안 건드렸다.
