...
구성 16(ca-)의 signal 정의(세션 48)  16 의 신호 = 독자에게 주어진 사실(감각·사물·타인 행동)이
                              ...
                              걸음 붙어 걸음).
짧은 결론형 재료는 복사되고 두 줄 재료는 소화된다(세션 49·50 실측, 두 번 재현)  재료 복사
                              경보(세션 49 4-A) 실측 — 짧은 결론형 재료(한 줄, "~은 ~다"류)가
                              힌트 본문에 그대로 베껴 나왔다: 세션 49 5/50(orc-axe 3 · fireball
                              2) · 세션 50 11/50(orc-axe 5 · fireball 6). bt-spear-range(박 님이
                              세션 49 에 확정한 두 줄 재료)는 두 세션 다 0/10 — 재료를 그대로
                              베끼지 않고 소화해 썼다. 세션 53 이 이 근거로 alley-hook·orc-axe·
                              fireball-shield 셋을 두 줄로 갈았다(bt-low-guard 는 한 줄 대조군
                              으로 남긴다 — 확장 안 함).
재료 규격(세션 53 채팅 확정)  두 줄 재료를 쓸 때: 앞줄 = 이 장면 안의 관찰거리 · 뒷줄 = 그
                              관찰의 값(대가·조건·갈래) · 주인공 이름 금지 · 수(해법) 금지 ·
                              격투기 일반론 금지 · 두 줄이 한 축에서 이어질 것.
                              ★ bt-alley-hook 은 확장이 아니라 **교체**다 — 기존 재료(오른손·
                              큰 훅)를 버리고 학습자 몫으로 넘겼다. 재측정에서 갈린다: 힌트가
                              "몸이 열린다" 쪽으로 옮겨 가면 성공, 여전히 손·궤도를 말하면
                              재료가 힌트에 안 닿은 것이고 이 문항은 원문이 이미 충분해 재료가
                              겉도는 자리라는 뜻이다(low-guard 와 정반대 진단).
...
```
1  (닫힘, 세션 51) 힌트 v3 hint_visible — 코드로 두 구멍을 다 막고 켜는 절차까지 준비 끝
                        경위(세션 48→51) — 세션 48 눈검사에서 signal(crystal 카드 제거·
                        gate-dinner 일치·fireball 통증 잡힘·alley-hook 과차단 없음)이 확인됐고,
                        같은 --hint 재측정에서 32/50 이 JSON 껍데기(통과율 100%가 이걸 못
                        봤다)로 나와 세션 49 가 껍데기 벗기기를 열었다. 세션 49 재측정(통과율
                        100% · 껍데기 43/50 · 재료 복사 5/50 · spear-range 0/10)에 숨은 구멍
                        둘 — 36번({"message":"…"}, 키가 feedback 이 아니라 벗기기를 피함) ·
                        44번("...2번 앞에..." 문장 번호 누출) — 을 세션 50 이 코드로 막았다
                        (벗기기 키 이름 무관화 · verifyHintV3 여섯째 제약 · computeHintV3 재시도
                        1회 + 'discarded' 캐시). 세션 50 재측정(통과율 98%, 정당한 폐기 1건 ·
                        껍데기 42/50 · **벗기지 못한 JSON 8/50, 전부 bare string** · 번호 누출
                        0/50 · 재료 복사 11/50 · spear-range 0/10)에서 마지막 구멍(bare string
                        JSON.parse 는 성공하지만 객체 분기 앞에서 안 걸림)이 드러나 이번 세션
                        (51)이 마저 막았다.
                        ★ 세션 51 — bare string 도 벗기고(객체 분기보다 먼저), 파싱 실패 판정을
                        '{' 뿐 아니라 '"' 로 시작할 때도 걸었다. **재측정 없이 켠다** — 결정적
                        변환이라 본문은 세션 50 에서 읽은 그대로고 따옴표만 벗겨진다(박 님
                        판단). 옛 캐시 정리(seed/clean-hint-v3-cache.sql — 세션 49 이전, 벗기기
                        신설 전에 route.ts 를 거쳐 껍데기째 저장된 캐시를 지운다. prompt_version
                        이 그대로라 캐시 키도 그대로라 지우지 않으면 켜는 순간 옛 껍데기가 뜬다)
                        → hint_visible=true(seed/update-hint-visible-on.sql) 순서로 SQL 만
                        준비했다 — DB 실행은 박 님 몫(아래 절차).
                        박 님 절차: ① seed/clean-hint-v3-cache.sql(먼저) → ② seed/update-hint-
                        visible-on.sql(1행 확인까지) → ③ 브라우저에서 힌트 카드가 따옴표 없이
                        뜨는지 눈검사.
                        판정선(세션 50 문구, 그대로 유효): "목적은 0 을 확인하는 게 아니라
                        고친 뒤에도 본문 결이 그대로인지 확인하는 것이다."
                        열린 채로 남기는 것(아래 미결 참고) — bt- 나머지 넷 재료 두 줄 확장(근거
                        는 섰으나 별도 세션에서 --hint 재측정과 묶어서) · 힌트 v3 프롬프트 평문
                        강제(JSON 금지, 계속 유보) · 타박조 표현 관찰(재제출 비교 피드백
                        프롬프트에 반영할 재료).
...
5  (닫힘, 세션 52) 재제출 비교 피드백 — 규칙 검사만(AI 없음, 비용 0)
                        ...
                        줄로 낸다, 별도 카드 안 만든다.
                        ★ 세션 53 보강 — diffChecks 가 이제 key 대조에 **rule 대조**를 더한다.
                        submissions.auto_result 는 제출 시점 판정이라 문항 설정이 바뀌면 과거
                        기록과 현재 기준이 어긋난다(실측 — maxChars fail 45건 중 rp-kongjwi-jar
                        11건이 34자 시절 기록인데 현재 설정은 68자다). rule 이 다르면 기준이
                        바뀐 것이지 학습자가 고친 게 아니므로 침묵한다 — '자주 하는 실수'
                        집계에도 그대로 필요한 대조다.
...
```
경위  세션 52 커밋(3814ac9) 위. 세션 49·50 실측(재료 복사 5/50 → 11/50, 두 줄인
  spear-range 는 두 번 다 0/10)이 "짧은 결론형 재료는 복사되고 두 줄 재료는
  소화된다"는 판정선을 세웠다 — 이번 세션이 그 근거로 bt- 나머지 셋을
  두 줄로 간다(low-guard 는 대조군으로 남긴다). 같은 세션에 재제출 비교
  피드백의 실측 구멍(rule 안 보고 key 만 대조해 문항 설정이 바뀐 과거 기록과
  현재 기준을 섞어 읽음) · problems 전수 검토 상태를 표시할 자리(reviewed)
  가 같이 들어왔다.

1. ai_hint_material 두 줄로(세 문항, 글자 하나도 안 바꿈)
  bt-alley-hook — **교체**(기존 "강태는 오른손만 쓰고, 주먹을 크게 돌린다."를
   버린다): "주먹은 뻗는 동안 그쪽 몸이 열린 채로 있다.\n열리는 자리는 뻗은
   팔이 지나간 아래쪽이고, 주먹이 돌아오기 전까지 그대로 있다."
  bt-orc-axe — 앞줄 유지, 뒷줄 추가: "...한 호흡이 걸린다.\n그 한 호흡은
   도끼가 어딘가에 박혀야 생기고, 빗나간 도끼는 땅에 박힌다."
  bt-fireball-shield — 앞줄도 교체: "화염구는 빚는 데 시간이 걸리고, 빚는
   동안 카엘의 두 손은 그 자리에 묶인다.\n막을 때마다 방패는 타들어 가지만
   카엘은 한 발도 물러서지 않는다."
  bt-low-guard·bt-spear-range 는 안 건드렸다 — low-guard 는 한 줄 대조군.
  재료 규격(세션 53 채팅 확정, 위 '정한 것') — 앞줄 = 관찰거리 · 뒷줄 = 그
  관찰의 값(대가·조건·갈래) · 주인공 이름 금지 · 수(해법) 금지 · 격투기
  일반론 금지 · 두 줄이 한 축에서 이어질 것. ★ alley-hook 은 확장이 아니라
  교체 — 재측정으로 성패가 갈린다(위 '정한 것' 참고, low-guard 와 정반대
  진단 축).
  seed/update-action-turn-v9.sql(신규, jsonb_set 멱등, 3건, 끝에 확인
  select) + seed/dump/problems.json 갱신(단일 출처).
  verify.ts — bt- 5건 전부 ai_hint_material 있음 · 세 문항은 '\n' 포함(두
  줄), 문안이 글자 하나도 안 바뀜 · low-guard 는 한 줄 그대로(대조군) ·
  새 재료 셋 다 자기 forbidWords 에 안 걸림 · update-action-turn-v9.sql
  존재·jsonb_set 문법·다른 두 문항 미접촉·문안 그대로 단언.

2. diffChecks — rule 대조 한 줄
  lib/scoring/resubmit.ts — `prev.key === curr.key` 짝짓기에 `prev.rule ===
  curr.rule` 을 더했다. submissions.auto_result 는 제출 시점 판정이라 문항
  설정이 바뀌면 과거 기록과 현재 기준이 어긋난다(실측 — maxChars fail 45건
  중 rp-kongjwi-jar 11건이 34자 시절 기록인데 현재 설정은 68자다). rule 이
  다르면 학습자가 고쳐서 통과한 게 아니라 기준이 바뀐 것이므로 침묵한다 —
  이 대조는 '자주 하는 실수' 집계에도 그대로 필요하다.
  verify.ts — 같은 key·다른 rule·fail→pass 조합이 빈 배열을 낸다는 픽스처
  추가.

3. problems.reviewed — 미검토 문항 표시(세션 53, 3-A 로 하네스 노출은 유보)
  seed_schema.sql — `alter table problems add column if not exists reviewed
  boolean not null default false;` 추가(새 DB 는 이 파일 하나로 컬럼이
  생긴다). seed/add-reviewed-column.sql(신규) — 같은 alter 문을 기존 DB 에
  얹는 델타(박 님이 직접 돌린다).
  ★ 초기값은 전부 false — 세션 이력으로 추정하지 않는다(급히 넘어간
  신설분도, 그 전에 전수로 본 것도 섞여 있다 — 틀린 초기값은 목록을
  거짓말로 만든다). 박 님이 눈으로 본 문항만 이후에 DB 에서 손으로 true 로
  올린다(이 JSON 파이프라인을 안 거친다).
  ★ 전체 157건 전부에 붙는다 — 활성 144·비활성 13 을 안 가린다("박 님이
  봤는가"는 활성 여부와 다른 축이다).
  seed/dump/problems.json — 157건 전부에 `"reviewed": false` 추가(CRLF·
  들여쓰기 등 기존 포맷을 그대로 유지, 다른 내용 변경 없음).
  scripts/gen-seed.ts — DumpProblem 에 reviewed 필드, problems insert 문의
  컬럼 목록·값에 reviewed 를 추가(sqlBool). seed_check.sql 에 미검토 건수
  집계(select reviewed, count(*) from problems group by 1) — 마지막
  "통과" select 앞에 둔다(Supabase 편집기는 마지막 결과만 보인다).
  ★ 3-A(하네스 노출 유보) — scripts/support-golden.ts 에는 [미검토] 태그·
  "미검토 n/N" 요약을 **이번엔 안 넣는다.** 초기값이 전부 false 라 첫 실행은
  50/50 이 나와 태그가 전 건에 붙고 정보가 0이다(박 님). 박 님이 true 를
  올리기 시작해 수가 갈리면 다음 세션 이후에 얹는다.
  verify.ts — 전체 157건 reviewed boolean·전부 false 단언 · seed_data.sql
  insert 컬럼 목록에 reviewed 가 157건 다 있다는 단언(count) · seed_check.sql
  의 미검토 집계 존재·통과 메시지보다 앞에 옴 · add-reviewed-column.sql·
  seed_schema.sql 이 같은 alter 문을 공유한다는 단언.

★ 문항 수 두 개를 섞지 않는다 — **전체 157 · 활성 144(전체 157 − 비활성
  13)**. verify.ts 의 "활성 문항 수 = 144" 단언은 그대로다. reviewed 컬럼과
  덤프 필드는 전체 157건 전부에 붙고, 전수 불변식(모범답안·forbidLabel 등)
  은 활성 144 대상 그대로다.

검증  tsc 0 · test:scoring(8001, 활성 144 단언 그대로 + 새 픽스처) 6248/0 ·
  check:numbers 0 · gen:seed 재생성 후 재실행 md5 동일(무변화) · next build
  통과. 물기(전부 확인 후 복원): bt-alley-hook 재료를 옛 값으로 되돌려 두
  단언(두 줄 여부·문안 그대로)이 잡는 것 확인 · diffChecks 의 rule 비교
  조건을 지워 rule 불일치 단언이 잡는 것 확인 · problems.json 첫 항목의
  reviewed 를 true 로 바꿔 "전부 false" 단언이 잡는 것 확인 · gen-seed.ts
  에서 reviewed 컬럼·값을 지우고 재생성해 seed_data.sql 단언이 잡는 것
  확인(재생성 후 md5 원복까지 확인) — 전부 복원 후 6248/0 재확인. 8001 만
  내렸다 — 8000(박 님 별도 터미널)은 안 건드렸다.
```
...
```
경위  세션 50 커밋(1c4b27d) 위. 박 님이 세션 50 의 코드(벗기기 키 이름 무관화·
  문장 번호 차단)로 --hint 를 재측정했다 — 통과율 98%(폐기 1건, bt-fireball-
  shield 37번: 인용이 「카엘의 힘은…」인데 답안 문장은 "상대 마법사 카엘의
  힘은…"으로 시작해 앞부분 규칙에 안 맞음 — 정당한 폐기, 실사용에선 재시도가
  건진다) · 껍데기 벗김 42/50 · **벗기지 못한 JSON 8/50, 전부 bare string**
  (`"「…」 … 봐."` 꼴) · 번호 누출 0/50(세션 50 이 막은 게 확인됐다) · 재료
  복사 11/50(orc-axe 5 · fireball 6 · spear-range 0/10). bare string 은
  JSON.parse 가 성공하는데(문자열도 유효한 JSON 값이다) 세션 50 의 객체 분기
  앞에서 안 걸려 원문(따옴표 포함)이 그대로 남았다 — 이번 세션이 이 마지막
  구멍을 막고, hint_visible 을 켜는 절차까지 준비했다.

1. 벗기기 — bare string 도 벗긴다
  lib/ai/observe.ts  unwrapHintV3Feedback — 파싱 결과가 **문자열**이면(객체
   분기보다 먼저 본다) trim 해서 text 로 쓴다(unwrapped: true). 문자열도
   객체도 아닌 스칼라(숫자·불리언·null)와 배열은 여전히 원문 유지·
   'malformed_json'(벗길 대상이 아니다). 객체 분기(문자열 필드 정확히 하나)
   는 세션 50 그대로. ★ 파싱 실패 시 'malformed_json' 판정을 원문이 '{' 뿐
   아니라 **'"' 로 시작할 때도** 걸었다 — 따옴표가 하나만 남거나 안쪽
   이스케이프가 깨진 응답이 오면 파싱이 실패하는데, 옛 규칙으론 false 로
   빠져 계기에서 사라진다(세션 49 "깨진 껍데기 0/50"과 같은 계열의 거짓
   안심).
  verify.ts 픽스처 셋 추가(기존 여덟 건은 글자 하나 안 건드리고 그대로 선다)
   — bare string 벗김(따옴표 없는 본문·unwrapped true) · 닫는 따옴표 없어
   파싱 실패(원문 그대로·malformed_json) · 42·true 같은 스칼라(원문 그대로·
   malformed_json, 둘 다 한 테스트에서 확인).

2. 옛 캐시 정리(켜기보다 먼저 — 이 세션 순서의 핵심)
  경위 — 벗기기는 judgeHintV3With **안에서만** 한다. 세션 48 눈검사 때 박 님
   실사용 답안은 route.ts 를 거쳐 캐시됐는데 그땐 벗기기가 없었다(당시 껍데기
   64%). PROMPT_VERSION_HINT_V3('hint-v3')가 그대로라 캐시 키(hash)도 그대로
   — hint_visible 을 켜는 순간 route.ts(599행)가 그 껍데기를 그대로 돌려준다.
   건당 재계산이 $0.003 이라 지우는 값이 아깝지 않다(박 님).
  seed/check-hint-v3-live.sql(신규, 읽기 전용) — ① prompt_version='hint-v3'
   행의 verdict 별 건수 ② judgment->>'text' 가 껍데기(JSON 객체·bare string)
   째 남은 행 수·최근 10건(hash·problem_id·본문 앞 40자) ③ verdict='discarded'
   (세션 50 신설) 최근 10건의 judgment->'reasons'.
  seed/clean-hint-v3-cache.sql(신규, 이 파일만 쓰기) — prompt_version=
   'hint-v3' **조건을 반드시 걸고**(support·tell·signal 캐시는 안 건드린다)
   껍데기째 남은 행만 delete, 끝에 0건 확인 select. 두 SQL 파일의 껍데기
   조건은 `(ltrim(judgment->>'text') like '{%' or ltrim(judgment->>'text')
   like '"%')`로 글자까지 같다 — ltrim 을 씌운 이유: 세션 48 당시
   judgeHintV3With(커밋 8b1b7e8)는 trim→코드펜스 제거→trim 순서라 보통 앞
   공백은 없지만, 닫는 펜스 정규식(`` /```$/ ``)이 문자열 끝에만 걸려 펜스
   뒤에 말이 붙은 응답은 앞에 잔여 문자가 남을 수 있다(박 님, 손해 없음).
   ★ ai_shadow_cache 권한은 service_role 에 select·insert 뿐(seed_schema.sql
   189행) — 이 delete 는 앱이 아니라 **박 님이 SQL 편집기에서** 돌린다. 앱
   코드엔 삭제 경로를 안 만든다.

3. hint_visible ON
  seed/update-hint-visible-on.sql(신규, 멱등) — `update system_flags set
   value = 'true' where key = 'hint_visible';` + 확인 select(1행 확인까지
   본다 — 0행이면 세션 46 의 update-hint-visible.sql 을 먼저 돌린다). value
   는 문자열이다(lib/ai/flags.ts asBoolean 이 boolean 과 'true'/'false'
   문자열을 받는다, 기존 파일 문법 그대로) — `::jsonb` 캐스팅을 안 쓴다.
   되돌리기 주석 한 줄(`'false'` 로 되돌리면 계산·캐시는 계속 서고 노출만
   꺼진다). ★ 이중 포장(벗긴 결과가 다시 '{'·'"' 로 시작)은 재귀로 안 벗긴다
   — 실측에 없다, 위 check-hint-v3-live.sql ②가 잡게 둔다(박 님).
  verify.ts — 신규 세 SQL 파일 존재·문안 단언(hint-visible-on 이 true 로
   켜는 유일한 파일임을 다른 두 off 파일과 대조, ::jsonb 미사용, 두 캐시
   SQL 의 껍데기 조건이 글자까지 같음, prompt_version 값이 PROMPT_VERSION_
   HINT_V3 상수와 일치).

검증  tsc 0 · test:scoring(8001, 활성 144 단언 그대로 + 새 픽스처) 6196/0 ·
  check:numbers 0 · gen:seed 무변화(이 세션은 시드·DB 를 안 건드렸다) · next
  build 통과 · `--hint --dry`(v3 문안 그대로 — 이번에도 프롬프트 미변경).
  물기(전부 확인 후 복원): unwrapHintV3Feedback 을 세션 50 판으로 되돌려 bare
  string 두 픽스처가 잡는 것 확인. 8001 만 내렸다 — 8000(박 님 별도 터미널)
  은 안 건드렸다.
★ DB 절차(박 님, 코드 배포 뒤) ① seed/clean-hint-v3-cache.sql ② seed/update-
  hint-visible-on.sql(1행 확인) ③ 브라우저에서 힌트 카드가 따옴표 없이
  뜨는지 눈검사. **재측정은 안 한다**(세션 50 에서 읽은 본문 그대로, 박 님
  판단).
```

### 끝난 것 — 세션 50 (힌트 v3 껍데기 벗기기 넓힘 · 문장 번호 누출 차단 · 계기 수치 정정)

```
경위  세션 49 커밋(34004cb) 위. 박 님이 세션 49 후속의 --hint 50건을 실측해 통과율
  100% 뒤에 숨은 구멍 둘을 더 찾았다 — ① 36번이 {"message": "…"} 로 와서 벗기기
  (feedback 키만 보던 규칙)를 피했다 — [벗김] 태그도 안 붙고 "통과"로 셌다. ②
  44번이 "...궤적을 2번 앞에 어떻게 비춰주면 좋을까?"로, 학습자 화면엔 없는
  문장 번호가 새어 나왔다(세션 47 이 막으려던 메타 지시의 다른 얼굴, HINT_V3_
  BANNED_WORDS 는 낱말 목록이라 숫자 패턴을 못 잡는다). 재측정한 세션 49 결과
  자체는 좋았다 — 통과율 100% · 껍데기 43/50 · 재료 복사 5/50(orc-axe 3 ·
  fireball 2) · spear-range 0/10(두 줄 재료는 안 베낀다). 이번 세션은 이 두
  구멍을 코드로 막고, "깨진 껍데기 0/50"이 36번을 놓친 이유(파싱이 사실은
  성공했다)를 계기 정의 자체에 반영했다. DB 작업 없음 — 프롬프트 문안도 이번
  세션은 안 건드렸다(벗기기·검증을 넓히면 무해해지고, 프롬프트까지 바꾸면 방금
  잰 결이 흔들려 hint_visible 판단이 흐려진다).

1. 껍데기 벗기기를 키 이름과 무관하게
  lib/ai/observe.ts  unwrapHintV3Feedback 규칙 교체 — JSON 파싱 성공·객체(배열
   아님)·**문자열 필드가 정확히 하나**면 키 이름과 무관하게 그 값을 trim 해서
   쓴다(unwrapped: true). 문자열 필드가 둘 이상이거나 하나도 없으면(숫자·객체·
   배열 필드는 안 센다) 원문 그대로 두고 unwrapped: 'malformed_json' — 파싱은
   됐지만 뭘 벗겨야 할지 모르는 경우다. 파싱 자체가 실패했는데 원문이 '{' 로
   시작하면 그대로 'malformed_json'(세션 49 그대로). ★ {"other":"x"} 는 문자열
   필드가 하나뿐이라 이제 벗긴다 — 키 이름을 안 보기로 한 규칙의 의도한
   결과다(세션 49 는 feedback 만 봐서 원문을 그대로 뒀었다).
  verify.ts 픽스처(신규, AI 호출 없음, 세션 49 네 건에 이어) — {"message":"…"}
   벗김(36번 재현) · {"feedback":"…","reason":"…"}(문자열 둘) 원문 그대로·
   malformed_json(넓힌 경계가 멈추는 자리) · {"feedback":"…","score":3}(문자열
   하나) 벗김 · 배열 원문 그대로·malformed_json · {"other":"x"} 이제 벗김(기존
   단언 갱신). 기존 feedback 정상·평문·JSON 뒤 말 붙어 파싱 실패 세 건은 그대로
   선다.
  scripts/support-golden.ts  --hint 개별 로그 태그 '깨진 껍데기' → '못 벗김'.

2. 문장 번호 누출 차단 + 2-A 재시도·discarded 캐시 정책
  lib/ai/prompt.ts  verifyHintV3 에 여섯째 제약 — `/\d+\s*번(째)?/` 가 걸리면
   reasons 에 '문장 번호 노출(메타 지시)'. "두 번"·"한 번"은 숫자가 아니라
   안 걸린다.
  lib/ai/observe.ts  judgeHintV3WithRetry(신규, export) — verifyHintV3 가
   실패하면 같은 프롬프트·같은 모델로 **한 번만** 재시도한다(outcome 자체가
   실패(call_failed·empty)한 경우는 재시도 안 함 — 재시도 대상은 "검증
   실패"뿐). **DB 를 안 만진다** — 캐시·ai_usage_log 는 호출부(route.ts) 몫,
   여기는 몇 번 불렀고 각 시도가 어땠는지(HintV3Attempt[], 1건 또는 2건)만
   돌려준다. verify.ts 가 가짜 call 로 재시도 횟수·최종 본문을 문다.
  app/api/grade/route.ts  computeHintV3 재작성(★ 세션 47 주석 "v2 와 같은
   모양"은 틀린 표현이었다 — 채팅 실측으로 정정, 박 님 결정) —
   · 캐시 조회가 verdict 'discarded' 를 보면 즉시 pending 으로 접는다.
   · judgeHintV3WithRetry 로 1~2회 시도 → 시도마다 ai_usage_log 정상 기록.
   · 최종 시도가 통과하면 그 본문을 verdict 'ok' 로 캐시.
   · 재시도까지 실패하면 verdict 'discarded' 로 캐시(judgment: {text: null,
     reasons}) — 같은 답안을 다시 안 부른다("재시도해도 안 되는 답안은 세
     번째도 안 될 확률이 높다", 박 님. 안 남기면 학습자가 카드를 열 때마다
     두 번씩 호출된다 — 비용이 새는 쪽이 더 나쁘다).
   · 학습자 화면은 어느 쪽이든 같다 — TrainClient 는 result.hint 유무로만
     카드를 그린다(verdict 를 안 본다, 확인 완료 — 2-A-1) · route.ts 도
     hint.verdict==='ok' && hint.text 일 때만 hintText 를 채운다 — discarded→
     pending 이 "힌트 없음, 1층 문구만"으로 정확히 떨어진다. 'none' 같은 새
     값을 안 만들었다.
   · ai_shadow_cache 스키마는 마이그레이션 불필요(2-A-2, 확인 완료) —
     seed_schema.sql 의 verdict 는 `text not null`(CHECK·enum 없음), 권한은
     `grant select, insert`뿐(update·upsert 없음) — 'discarded' 문자열을 그냥
     insert 한다. 같은 해시 재삽입은 기본키 충돌이 나지만 캐시 조회가 먼저라
     정상 흐름에선 안 난다 — 경합으로 충돌해도 기존처럼 console.error 만
     남기고 진행(판정에 영향 없음).
   · v2(computeHintV2)는 안 건드렸다 — 지금 아무 문항도 안 쓴다.
  verify.ts  judgeHintV3WithRetry 픽스처 셋(첫 시도 실패→재시도 통과 · 두
   시도 다 실패→최종 check.ok===false · 첫 시도 통과→둘째 호출 없음, 호출
   횟수로 확인) · route.ts computeHintV3 소스 정규식 단언(judgeHintV3WithRetry
   호출·discarded 캐시 즉시 pending·시도마다 ai_usage_log·'ok'/'discarded'
   두 insert 가 같은 hash·prompt_version 공유) · TrainClient 힌트 카드가
   result.hint 유무로만 그려지는지(verdict 미참조) 단언 · verifyHintV3 여섯째
   제약 픽스처 넷("2번 앞에" 실패·"2번째 문장" 실패·"두 번 읽어 봐" 통과·"3번
   문항" 실패).

3. 하네스 수치 정의 정정 + 3-A 번호 누출 줄
  scripts/support-golden.ts  --hint 요약 "껍데기 벗김 n/50 · 벗기지 못한 JSON
   m/50"으로 문구 교체('malformed_json' 은 "파싱 실패"가 아니라 "벗기지 못한
   JSON"이라는 재정의 — 세션 49 "깨진 껍데기 0/50"이 36번을 못 잡은 게 그래서
   였다). "번호 누출 n/50(문항별 내역)" 줄 신설(verifyHintV3 여섯째 제약
   기준). 재료 복사 줄에 6자 연속 표현 겹침만 잰다는 한계 주석 한 줄(세션 49
   21번은 뜻이 같아도 표현이 달라 안 걸렸다). 요약 위에 "하네스는 route.ts 를
   안 거친다 — 재시도 경로를 안 탄다, 이 수치는 재시도 전 1차 발생률이다"
   주석 추가(3-A).

4. 2-B 정규식 안전 확인(완료, 예외 불필요)
  채팅이 bt- 5문항 원문·지시문·scoring_config · 모범답안 10건 · data/probe/
   set_b_nak.json · set_d_tell.json 전부에서 `\d+\s*번(째)?` 0건을 실측했다 —
   정당한 인용이 걸릴 자리가 없어 예외 처리를 안 넣는다. verify.ts 에 이
   전제를 불변식으로 박았다(bt- 5건 개별 + 모범답안 10건 + 두 golden 파일
   전체 텍스트) — 나중에 원문에 그런 표현이 들어오면 이 단언이 먼저 문다.

검증  tsc 0 · test:scoring(8001, 활성 144 단언 그대로 + 새 픽스처) 6182/0 ·
  check:numbers 0 · gen:seed 무변화(이 세션은 시드·DB 를 안 건드렸다) · next
  build 통과 · `--hint --dry`(v3 문안 그대로 — 이 세션도 프롬프트 미변경).
  8001 만 내렸다 — 8000(박 님 별도 터미널)은 안 건드렸다.
```
...
bt-alley-hook 재료 교체가 힌트에 실제로 닿는지 — 세션 53 이 alley-hook·orc-axe·
  fireball-shield 를 두 줄로 갈았다(bt-low-guard 는 한 줄 대조군으로 남긴다).
  alley-hook 은 확장이 아니라 교체다(오른손·큰 훅 정보를 없앴다) — 다음
  --hint 재측정에서 힌트가 "몸이 열린다" 쪽으로 옮겨 가면 성공, 여전히 손·
  궤도를 말하면 이 문항은 원문이 이미 충분해 재료가 겉도는 자리라는 뜻이다
  (low-guard 와 정반대 진단, 위 '정한 것' 참고). 아직 재측정 안 함.
...
hint_visible — 코드는 끝, 실행만 남았다(세션 51) — 세션 50 이 코드로 두 구멍
  (키 이름 무관 벗기기·문장 번호 재시도+discarded 캐시)을, 세션 51 이 마지막
  구멍(bare string 미벗김)을 막았다. **재측정 없이 켠다**(결정적 변환이라 세션
  50 에서 읽은 본문 그대로다, 박 님 판단) — 박 님 절차: ① seed/clean-hint-v3-
  cache.sql(세션 49 이전 껍데기째 캐시 정리, 먼저) → ② seed/update-hint-
  visible-on.sql(1행 확인까지) → ③ 브라우저 눈검사.
힌트 v3 프롬프트에 평문 강제(JSON 금지) 추가 — 세션 49·50·51 셋 다 일부러 안
  넣었다(재료·검증 변경과 같은 커밋에서 프롬프트를 바꾸면 재측정이 뭘 잰 건지
  흐려진다). hint_visible 을 켜고 실사용을 지켜본 뒤 필요하면 다음 세션에서
  넣는다 — 코드로 이미 두 구멍(껍데기·번호 누출)을 다 막아 급하지 않다.
discarded 캐시는 프롬프트 버전을 올려야 풀린다(세션 50, 2-A-3) — 해시가
  `${normalized} ${problemId} ${PROMPT_VERSION_HINT_V3} ${model}` 이라, 같은
  버전 안에서 hint-v3 문안만 손보면 이미 discarded 로 캐시된 답안들은 계속
  힌트를 못 받는다(문안이 바뀌어도 캐시 키가 안 바뀐다). 문안을 고칠 때는
  버전을 같이 올리거나 `delete from ai_shadow_cache where verdict='discarded'`
  를 함께 낸다.
세션 49 이전 힌트 v3 캐시는 벗기기 전 본문이다(세션 51 발견) — 벗기기
  (unwrapHintV3Feedback)는 judgeHintV3With **안에서만** 한다. route.ts 를 거쳐
  이미 저장된 캐시는 벗기기 신설 전 그대로다. PROMPT_VERSION_HINT_V3 가 안
  바뀌면 캐시 키(hash)도 그대로라 옛 껍데기가 그대로 나온다 — 벗기기 규칙을
  바꿀 때는 seed/clean-hint-v3-cache.sql(또는 그 조건)을 함께 돌린다.
재시도가 돌면 consumeAiQuota 는 한 번인데 Gemini 는 두 번 간다(세션 50 리뷰,
...
144→157 문항 중 박 님이 전수로 검토하지 못한 것이 상당수다(세션 53, problems.
  reviewed 신설 계기) — 규모의 증상이지 결함이 아니다. 전부 훑는 세션을 따로
  잡지 않고 닿는 순서대로 올린다 — 힌트 재측정·실사용에서 걸리는 문항이
  실제로 손봐야 할 문항이고, 안 걸리면 미검토여도 실해가 없다(박 님).
reviewed 가 전부 false 인 동안은 하네스 표시가 무의미하다(세션 53, 3-A) —
  초기값이 전부 false 라 --hint 첫 실행은 50/50 이 나오고 전 건에 [미검토]
  태그가 붙어 정보가 0이다. scripts/support-golden.ts 에는 이번 세션에
  태그·요약을 안 넣었다 — 박 님이 검토 표시를 올리기 시작해 수가 갈린 뒤에
  --hint 로그·요약에 얹는다. seed_check.sql 은 지금도 건수를 낸다(select
  reviewed, count(*) from problems group by 1).
```
